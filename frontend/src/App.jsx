  ...
  if (student.status === "in_progress") {
    return (
      <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-bold text-amber-700">
        Хийж байна · Үе {student.playingStage || "?"}
      </span>
    );
  }
  return (
...
function roomLabel(room) {
  if (room.status === "stopped")
    return { text: "Зогссон", cls: "bg-orange-100 text-orange-700" };
  if (room.gameStarted)
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-indigo-800 via-violet-700 to-purple-800 p-4 text-white">
      <form
        ...
        className="w-full max-w-md rounded-3xl bg-white/10 p-6 shadow-2xl ring-1 ring-white/20 backdrop-blur"
      >
        <p className="text-xs font-semibold uppercase tracking-[0.28em] text-indigo-100">
          Math Battle
        </p>
        <h1 className="mt-2 text-2xl font-black">Багш нэвтрэх</h1>
        <p className="mt-1 text-sm text-indigo-100">
          Ангийн хяналтын самбар руу орохын тулд нэвтэрнэ үү.
        </p>

        <label className="mt-6 block text-sm font-semibold text-indigo-50">
          Нэр
        </label>
        <input
          value={username}
        ...
        <label className="mt-4 block text-sm font-semibold text-indigo-50">
          Нууц үг
        </label>
        <input
          type="password"
        ...
        {error && (
          <p className="mt-3 rounded-xl bg-rose-500/20 px-3 py-2 text-sm text-rose-100">
            {error}
          </p>
        )}
...
function App() {
  ...
  const liveCount = useMemo(
    () => classes.filter((c) => c.gameStarted).length,
    [classes],
  );
  const selected =
    classes.find((c) => c.code === selectedCode) || classes[0] || null;
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-6 sm:flex-row sm:items-center sm:justify-between sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="rounded-xl bg-rose-500/80 px-4 py-2 text-sm font-semibold hover:bg-rose-500"
            >
              Гарах
            </button>
          </div>
        ...
        <section className="mt-8 rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          <div className="mb-5">
            ...
            <p className="text-sm text-slate-500">
              QR код, join холбоос, сурагчийн бүртгэл · 2 сек тутамд
              шинэчлэгдэнэ
            </p>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        handleDeleteClass(item.code, item.className)
                      }
            ...
            <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
              <div>
                <h2 className="text-xl font-bold text-slate-900">
                  {selected.className} · шууд оноо
                </h2>
                <p className="text-sm text-slate-500">
                  Код {selected.code} · Дууссан {selected.finishedCount || 0}/
                  ...
                  <tbody>
                    {(selected.students || []).map((s) => (
                      <tr key={s.name} className="border-b border-slate-100">
                        ...
                        <td className="py-3 px-2 text-center text-lg font-black text-indigo-700">
                          {s.scores?.total ?? 0}
                        </td>
          ...
          <div className="rounded-3xl bg-gradient-to-br from-indigo-600 to-violet-700 p-6 text-white shadow-lg">
            ...
            <div className="mt-6 rounded-2xl bg-white p-5 text-center text-slate-900 shadow-inner">
              {selected?.qrDataUrl ? (
                ...
                <div className="mx-auto flex h-32 w-32 items-center justify-center rounded-xl bg-slate-100 text-4xl shadow-sm">
                  QR
                </div>
              )}
            </div>
            ...
            {selected && (
              <p className="mt-3 text-center font-mono text-lg font-black tracking-widest">
                {selected.code}
              </p>
            )}
          </div>
